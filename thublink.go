...
func StartFull(url, title, ico, ua, devPath string, max, mb, ib, show bool, width, height int, finish win32.FinishCallback, save win32.SaveCallback,
	...
	catchSet := getSet(set)
	log.SetUA(ua)
	return win32.StartBlinkMain(url, title, ico, ua, devPath, max, mb, ib, show, max, width, height, jsFunc, forms, catchSet, save, finish)
}
...
func RunNoWindow(ico, title, ua, devPath string, max, mb, ib bool, width, height int,
	...
	log.SetUA(ua)
	return win32.StartBlinkMain("", title, ico, ua, devPath, max, mb, ib, width, height, jsFunc, nil, getSet(set), nil, nil)
}
