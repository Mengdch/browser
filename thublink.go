...
func Start(url, title, ico, ua, devPath string, max bool, width, height int) error {
	// 加密算法，服务
	return StartFull(url, title, ico, ua, devPath, max, true, true, true, width, height, nil, nil, map[int32]func(string) string{
		1: func(string) string {
			return "true"
		},
	}, nil, nil, nil)
}
func StartFull(url, title, ico, ua, devPath string, max, mb, ib, show bool, width, height int, finish win32.FinishCallback, save win32.SaveCallback,
	jsFunc map[int32]func(string) string, forms map[string]win32.FormProfile, set func(uintptr), domains []string) error {
	catchSet := getSet(set)
	log.SetUA(ua)
	return win32.StartBlinkMain(url, title, ico, ua, devPath, max, mb, ib, show, max, width, height, jsFunc, forms, catchSet, save, finish, domains)
}
...
func RunNoWindow(ico, title, ua, devPath string, max, mb, ib bool, width, height int,
	...
	log.SetUA(ua)
	return win32.StartBlinkMain("", title, ico, ua, devPath, max, mb, ib, width, height, jsFunc, nil, getSet(set), nil, nil, nil)
}
